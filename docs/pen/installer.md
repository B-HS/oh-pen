...
- `opencode debug agents`는 정적 agent 등록·권한·설치 모델을 확인한다. 호출별 child 모델 전환은 session 상태이므로 실제 native 실행의 child session과 assistant 메시지에서 별도로 확인한다.
- installer의 verify는 plugin 파일의 manifest 등록과 SHA-256을 검사한다. plugin 활성화와 prompt admission hook의 모델 적용은 새 OpenCode session의 실제 child smoke test 경계다.
...
│   ├── opencode-plugin.ts native child 호출별 모델·결과 검증 plugin
│   ├── opencode-plugin-native-subagent.ts native subagent 입력·metadata 계약
